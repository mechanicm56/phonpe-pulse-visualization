...
host = database_credentials["host"]; user = database_credentials["user"]
password = database_credentials["password"]; database = database_credentials["database"]
...
def get_dataframe(table_name):
    ...
    df = pd.DataFrame(
    data,
    columns=[i[0].capitalize() for i in cursor.description]
)
    # df['Year'] = df['Year'].astype(str)
    print(df)
    return df

agg_insur_df = agg_trans_df = agg_user_df = map_trans_df = map_user_df = \
    top_insur_dist_df = top_insur_pin_df = top_trans_dist_df = top_trans_pin_df = top_user_dist_df = top_user_pin_df = None
    
table_names = [
    'agg_insur', 'agg_trans', 'agg_user', 
    'map_insur', 'map_trans', 'map_user', 
    'top_insur_dist', 'top_insur_pin',
